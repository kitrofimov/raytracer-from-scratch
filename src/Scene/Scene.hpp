#pragma once
#include <vector>
#include <memory>
#include "Sphere/Sphere.hpp"
#include "Light/Light.hpp"
#include "utils/color/color.hpp"
#include "utils/vec/vec3.hpp"

class Camera;

class Scene
{
public:
    Scene(std::vector<std::unique_ptr<Sphere>> &objects,
          std::vector<std::unique_ptr<LightSource>> &light_sources,
          color_t background_color = (color_t) {0, 0, 0, 255});
    color_t cast_ray(vec3d origin, vec3d direction, Camera& camera);
    double calculate_light_intensity(vec3d point, vec3d normal, Camera camera, std::unique_ptr<Sphere>& p_object);
private:
    std::vector<std::unique_ptr<Sphere>> objects;
    std::vector<std::unique_ptr<LightSource>> light_sources;
    color_t background_color;
};
